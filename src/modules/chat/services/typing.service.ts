...
import { TypingEvent, TypingRepository } from '../repositories/typing.repository';

export function useSetTyping() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ chatId, userId, isTyping }: { chatId: string; userId: string; isTyping: boolean }) => {
      await TypingRepository.setTyping(chatId, userId, isTyping);
    },
    onSuccess: (_data, variables) => {
      // Invalidate typing users for this chat
      queryClient.invalidateQueries({ queryKey: ['typing-users', variables.chatId] });
    },
  });
}
